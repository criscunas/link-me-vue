...
export default {
  created() { 
    HTTP
      ...
      .then((res) => {
        console.log(res)
        this.backgroundColor = res.data.bg_color
        this.usernameColor = res.data.username_color
        this.textColor = res.data.text_color
        this.bioColor = res.data.bio_color
      })
  },
  setup() {
...
<template>
  ...
  <div class="mt-4">
    ...
    <div class="grid grid-cols-2 gap-y-4">
      <FormKit type="color" v-model="backgroundColor" label="Select a background color" />
      <FormKit type="color" v-model="textColor" label="Select a caption color" />
      <FormKit type="color" v-model="usernameColor" label="Select a username color" />
      <FormKit type="color" v-model="bioColor" label="Select a bio color" />
    </div>
    <div class="mt-4 mockup-phone w-[290px] flex justify-center flex-col">
      ...
      <div class="display">
        <div class="artboard artboard-demo phone-1" :style="{ backgroundColor: backgroundColor }">
          ...
          <p :style="{ color: usernameColor }" class="font-semibold text-xl mt-2">@{{ username }}</p>
          ...
          <ul class="mt-6">
            <li v-for="(link, index) in links" :key="index" class="list-none flex items-center py-2 gap-4">
              ...
              <p :style="{ color: textColor }" class="font-semibold">{{ link.caption }}</p>
            </li>
          </ul>
        </div>
      </div>
